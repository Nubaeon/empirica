...
bootstrap_session(
    ai_id="rovodev",  # or "qwen", "minimax", "gemini", etc.
...
checkpoint = load_git_checkpoint("latest:active:rovodev")
...
state = get_epistemic_state("latest:active:rovodev")
...
- `latest:active:rovodev` - Most recent active session for your AI (recommended!)

**Option 2: Load Handoff Report (98.8% token savings for multi-agent work)**
```python
# Query handoff reports by AI
handoffs = query_handoffs(ai_id="rovodev", limit=1)
...
1. Try to load checkpoint: `load_git_checkpoint("latest:active:rovodev")`
