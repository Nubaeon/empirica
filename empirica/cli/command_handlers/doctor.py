...
def check_empirica_mcp() -> Check:
    ...
    if not path:
        return Check("empirica-mcp on PATH", WARN, "",
...
def check_claude_code_cli() -> Check:
    ...
    path = _which("claude")
    if not path:
        return Check("Claude Code CLI on PATH", WARN, "",
...
def check_empirica_folder(cwd: Path | None = None) -> Check:
    ...
    if not folder.exists():
        return Check(".empirica/ folder", WARN, f"not present at {cwd}",
                     "Run `empirica project-init` in a project directory")
...
def check_project_yaml(cwd: Path | None = None) -> Check:
    ...
    if not pyaml_path.exists():
        return Check("project.yaml present + has ai_id", WARN, f"not at {pyaml_path}",
                     "Run `empirica project-init`",
                     data={"path": str(pyaml_path)})
    try:
        import yaml
    except ImportError:
        return Check("project.yaml present + has ai_id", WARN, "PyYAML not installed",
                     "pip install pyyaml")
    try:
        data = yaml.safe_load(pyaml_path.read_text(encoding="utf-8")) or {}
    except Exception as e:
        return Check("project.yaml present + has ai_id", FAIL, f"unparseable: {e}",
                     "Inspect/repair the file manually")
    if not isinstance(data, dict):
        return Check("project.yaml present + has ai_id", FAIL, "not a YAML object",
                     "Inspect/repair the file manually")
    ai_id = data.get("ai_id")
    if not ai_id:
        return Check("project.yaml present + has ai_id", WARN, f"{pyaml_path} (no ai_id)",
                     "Re-run `empirica project-init --force` or edit project.yaml to add ai_id",
                     data={"path": str(pyaml_path)})
    return Check("project.yaml present + has ai_id", PASS, f"ai_id={ai_id}",
                 data={"path": str(pyaml_path), "ai_id": ai_id,
...
def check_sessions_db(cwd: Path | None = None) -> Check:
    ...
    if not db_path.exists():
        return Check("sessions DB accessible", WARN, f"not at {db_path}",
    ...
    try:
        ...
        return Check("sessions DB accessible", FAIL, f"sqlite error: {e}",
                     "DB may be corrupt — back up and recreate via `empirica session-create`",
                     data={"path": str(db_path)})
    if not row:
        return Check("sessions DB accessible", WARN, "no `sessions` table",
                     "Run any `empirica session-create` to bootstrap schema",
                     data={"path": str(db_path)})
    return Check("sessions DB accessible", PASS, f"{db_path} ({session_count} sessions)",
...
def check_cortex_creds() -> Check:
    ...
    if missing:
        return Check("Cortex credentials configured", WARN, f"missing: {', '.join(missing)}",
    ...
    return Check("Cortex credentials configured", PASS, f"url={url} (api_key present)",
                 data={"url": url, "has_api_key": True})
...
def check_cortex_auth() -> Check:
    ...
    if not (url and api_key):
        return Check("Cortex auth + mesh fields", SKIP, "no creds configured",
                     "See 'Cortex credentials configured' check above")
    me_url = f"{url}/v1/users/me"
    status, body = _http_get(me_url, headers={"Authorization": f"Bearer {api_key}"})
    if status == -1:
        return Check("Cortex auth + mesh fields", WARN, f"{me_url} unreachable: {body}",
                     "Check network / VPN / CORTEX_REMOTE_URL")
    if status == 401:
        return Check("Cortex auth + mesh fields", FAIL, f"{me_url} → 401 Unauthorized",
                     "Rotate api_key via cortex admin, then update ~/.empirica/credentials.yaml")
    if status >= 400:
        return Check("Cortex auth + mesh fields", FAIL, f"{me_url} → {status}",
                     "Check cortex server logs",
                     data={"status": status})
    try:
        payload = json.loads(body) if body else {}
    except json.JSONDecodeError:
        return Check("Cortex auth + mesh fields", WARN, f"{status} but malformed JSON",
                     data={"status": status})
    mesh_fields = {k: payload.get(k) for k in ("org_id", "tenant_slug", "mesh_id_prefix")}
    missing = [k for k, v in mesh_fields.items() if not v]
    if missing:
        return Check("Cortex auth + mesh fields", WARN,
    ...
    return Check("Cortex auth + mesh fields", PASS,
...
def check_ntfy_creds() -> Check:
    ...
    if missing:
        return Check("ntfy credentials configured", WARN, f"missing: {', '.join(missing)}",
                     "Run `empirica setup-claude-code` wizard or hand-edit ~/.empirica/credentials.yaml")
    return Check("ntfy credentials configured", PASS,
...
def check_ntfy_auth() -> Check:
    ...
    url = cfg.get("url")
    if not url:
        return Check("ntfy reachable + auth", SKIP, "no ntfy url configured",
                     "See 'ntfy credentials configured' above")
    ...
    if status == -1:
        return Check("ntfy reachable + auth", WARN, f"{account_url} unreachable",
                     "Check network / VPN / NTFY_URL")
    if status in (200, 201):
        return Check("ntfy reachable + auth", PASS, f"{account_url} → {status}",
                     data={"status": status})
    if status in (401, 403):
        return Check("ntfy reachable + auth", FAIL, f"{account_url} → {status} (bad creds)",
                     "Verify token / user / password in credentials.yaml",
                     data={"status": status})
    return Check("ntfy reachable + auth", WARN, f"{account_url} → {status}",
                 data={"status": status})
...
def check_loops_registered() -> Check:
    ...
    if rc != 0:
        return Check("canonical loops registered", WARN, "`empirica loop list` failed",
    ...
    if not loops:
        return Check("canonical loops registered", WARN, "no loops registered",
    ...
    return Check("canonical loops registered", PASS, f"{len(loops)} loop(s): {', '.join(names[:5])}",
                 data={"loops": names})
...
def check_mcp_config() -> Check:
    ...
    if not found_configs:
        return Check("MCP servers configured", WARN, "no MCP config found",
    ...
    if has_empirica or has_cortex:
        ...
        return Check("MCP servers configured", PASS, " | ".join(detail_parts),
                     data={"configs": found_configs, "has_empirica": has_empirica,
                           "has_cortex": has_cortex})
    return Check("MCP servers configured", WARN,
...
def check_git_remote(cwd: Path | None = None) -> Check:
    ...
    if not (cwd / ".git").exists():
        return Check("git repo + remote", WARN, "not a git repo",
                     "git init && git remote add origin <url>")
    rc, out, _ = _run(["git", "-C", str(cwd), "remote", "-v"])
    if rc != 0 or not out:
        return Check("git repo + remote", WARN, "no remote configured",
                     "git remote add origin <url>  — sync_push needs a remote")
    remotes = [line.split()[0] for line in out.splitlines() if line]
    return Check("git repo + remote", PASS, f"{len(set(remotes))} configured",
...
def check_cortex_reachability() -> Check:
    ...
    for probe in (f"{base}/v1/health", f"{base}/cortex/health", base):
        ...
        if status >= 0:
            ...
            return Check("Cortex reachability", PASS, f"{probe} → {status}",
                         data={"url": cortex_url, "probe": probe, "status": status})
        # Network/DNS failure — try next probe in case it's a path-specific block.
        last_error = body
    return Check("Cortex reachability", WARN, f"{base} unreachable: {last_error}",
...
def handle_doctor_command(args: Any) -> int:
    ...
    if output_format == "human":
        ...
        payload = {
            "ok": fails == 0,
            "summary": {"total": len(checks), "pass": passed, "warn": warns,
