...
def add_checkpoint_parsers(subparsers):
    ...
    mistake_log_parser.add_argument('--mistake', required=True, help='Short title — what was done wrong. Rendered as markdown.')
    mistake_log_parser.add_argument('--why-wrong', required=True, help='Short explanation of why it was wrong. Rendered as markdown.')
    ...
    mistake_log_parser.add_argument('--prevention', help='Short — how to prevent this mistake in the future. Rendered as markdown.')
    mistake_log_parser.add_argument('--description', help='Optional rich markdown body — full account: trigger, signals you missed, recovery path, related findings/dead-ends. Rendered in extension and skill surfaces.')
    ...
    mistake_log_parser.add_argument('--source', action='append', dest='source_ids', help='Source ID (from source-add). Repeatable for multiple sources.')
    ...
    deadend_log_parser.add_argument('--approach', required=False, help='Short title — what approach was tried. Rendered as markdown; use --description for the full story.')
    ...
    assumption_log_parser.add_argument('--assumption', required=False, help='Short title — the assumption being made. Rendered as markdown; use --description to record the basis for the confidence.')
    assumption_log_parser.add_argument('--description', help='Optional rich markdown body — what would verify or falsify the assumption, why you\'re leaning toward the stated confidence, how brittle it is. Rendered in extension and skill surfaces.')
    ...
    assumption_log_parser.add_argument('--source', action='append', dest='source_ids', help='Source ID (from source-add). Repeatable for multiple sources.')
    ...
    decision_log_parser.add_argument('--choice', required=False, help='Short title — the choice made. Rendered as markdown; use --description for the full deliberation.')
    decision_log_parser.add_argument('--alternatives', required=False, help='Alternatives considered (comma-separated or JSON array)')
    decision_log_parser.add_argument('--rationale', required=False, help='Short rationale — why this choice was made. Rendered as markdown; use --description for extended reasoning.')
    decision_log_parser.add_argument('--description', help='Optional rich markdown body — extended reasoning, trade-offs table, what would change this decision, related findings. Rendered in extension and skill surfaces.')
    ...
    decision_log_parser.add_argument('--source', action='append', dest='source_ids', help='Source ID (from source-add) for external citations. Repeatable.')
...
Example:
  echo '{"deletions": [{"type": "finding", "id": "abc123"}], "reason": "Stale test data"}' | empirica delete-artifacts -
    ...
    goals_create_parser.add_argument('--description', help='Optional rich body — context, motivation, success-criteria detail (8000 char cap)')
    ...
    goals_create_parser.add_argument('--status', choices=['planned', 'in_progress', 'blocked'], default='in_progress',
