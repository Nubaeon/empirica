...
def add_onboarding_parsers(subparsers):
    ...
    onboard_parser = subparsers.add_parser(
        "onboard", help="Interactive introduction to Empirica (recommended for first-time users)"
    )
...
Run this after 'brew install empirica' or 'pip install empirica'.
    ...
    # Tenant metadata escape hatches — override the cortex tenant-metadata fetch
    ...
    setup_cc_parser.add_argument(
        "--org-id", default=None, help="Override tenant org_id (skip cortex tenant-metadata fetch for this field)"
    )
    setup_cc_parser.add_argument(
        "--tenant-slug", default=None, help="Override tenant_slug (skip cortex tenant-metadata fetch for this field)"
    )
    setup_cc_parser.add_argument(
        "--mesh-id-prefix",
        default=None,
        help="Override mesh_id_prefix (skip cortex tenant-metadata fetch for this field)",
    )
    setup_cc_parser.add_argument(
        "--skip-claude-md", action="store_true", help="Skip CLAUDE.md installation (keep existing system prompt)"
    )
    setup_cc_parser.add_argument(
        "--output", choices=["human", "json"], default="human", help="Output format (default: human)"
    )
    setup_cc_parser.add_argument("--verbose", action="store_true", help="Show detailed output")
...
One-shot:
  ...
  empirica release --dry-run          # preview without executing
    ...
    release_parser.add_argument("--dry-run", action="store_true", help="Preview changes without executing")
    release_parser.add_argument(
        "--prepare", action="store_true", help="Merge to main, build, and test — but do NOT publish"
    )
    release_parser.add_argument(
    ...
    release_parser.add_argument("--old-version", help="Previous version for broad sweep replacement (e.g. 1.5.6)")
