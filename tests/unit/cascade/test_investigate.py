...
class TestInvestigatePhase:
    ...
    def test_investigate_phase_with_rounds(self):
        ...
        assert assessment_r1.task == task
        assert assessment_r2.task == task
    ...
    def test_investigation_process(self):
        ...
        assessment_with_gaps = EpistemicAssessment(
            ...
            know=VectorState(0.4, "Limited knowledge about database connection issues"),
            do=VectorState(0.6, "Moderate capability for debugging"),
            context=VectorState(0.5, "Some context about the error"),
            foundation_confidence=0.5,
            clarity=VectorState(0.55, "Moderate clarity"),
            coherence=VectorState(0.65, "Reasonable coherence"),
            signal=VectorState(0.6, "Some priority clarity"),
            ...
            change=VectorState(0.45, "Limited change tracking"),
            ...
            assessment_id="gaps_investigation_test"
        )
    ...
    def test_investigation_strategy_generation(self):
        ...
        clarity_gap_assessment = EpistemicAssessment(
            ...
            context=VectorState(0.7, "Good context"),
            foundation_confidence=0.7,
            clarity=VectorState(0.4, "LOW CLARITY - main issue"),
            coherence=VectorState(0.7, "Good coherence"),
            signal=VectorState(0.65, "Good signal"),
            density=VectorState(0.5, "Moderate density"),
            ...
            recommended_action=Action.INVESTIGATE,
            assessment_id="clarity_gap_strategy_test"
        )
        ...
        knowledge_gap_assessment = EpistemicAssessment(
            engagement=VectorState(0.75, "Good engagement"),
            engagement_gate_passed=True,
            know=VectorState(0.3, "LOW KNOWLEDGE - main issue"),
            do=VectorState(0.65, "Good capability"),
            context=VectorState(0.6, "Moderate context"),
            ...
            state=VectorState(0.6, "Moderate state awareness"),
            change=VectorState(0.65, "Good change tracking"),
            completion=VectorState(0.6, "Moderate completion awareness"),
            impact=VectorState(0.6, "Moderate impact understanding"),
            execution_confidence=0.62,
            uncertainty=VectorState(0.6, "High uncertainty"),
            overall_confidence=0.62,
            recommended_action=Action.INVESTIGATE,
            assessment_id="knowledge_gap_strategy_test"
        )
        ...
        context_gap_assessment = EpistemicAssessment(
            ...
            know=VectorState(0.7, "Good domain knowledge"),
            do=VectorState(0.7, "Good capability"),
            context=VectorState(0.35, "LOW CONTEXT - main issue"),
            ...
            density=VectorState(0.5, "Moderate density"),
            comprehension_confidence=0.66,
            state=VectorState(0.4, "LOW STATE - secondary issue"),
            change=VectorState(0.65, "Good change tracking"),
            completion=VectorState(0.65, "Good completion awareness"),
            impact=VectorState(0.65, "Good impact understanding"),
            execution_confidence=0.63,
            uncertainty=VectorState(0.45, "Moderate uncertainty"),
            overall_confidence=0.64,
            recommended_action=Action.INVESTIGATE,
            assessment_id="context_gap_strategy_test"
        )
    ...
    def test_investigation_loop_simulation(self):
        ...
        current_assessment = EpistemicAssessment(
            engagement=VectorState(0.75, "Good engagement"),
            engagement_gate_passed=True,
            know=VectorState(0.4, "Low knowledge"),
            do=VectorState(0.5, "Moderate capability"),
            context=VectorState(0.45, "Low context"),
            foundation_confidence=0.45,
            clarity=VectorState(0.5, "Moderate clarity"),
            coherence=VectorState(0.55, "Moderate coherence"),
            signal=VectorState(0.45, "Low signal"),
            ...
            recommended_action=Action.INVESTIGATE,
            assessment_id="loop_simulation_test"
        )
        ...
        while current_assessment.overall_confidence < cascade.action_confidence_threshold:
            ...
            current_assessment = EpistemicAssessment(
                engagement=VectorState(0.75, f"Maintained engagement after round {investigation_rounds}"),
                engagement_gate_passed=True,
                know=VectorState(0.4 + (investigation_rounds * 0.15), f"Improved knowledge after round {investigation_rounds}"),
                do=VectorState(0.5 + (investigation_rounds * 0.12), f"Improved capability after round {investigation_rounds}"),
                context=VectorState(0.45 + (investigation_rounds * 0.14), f"Improved context after round {investigation_rounds}"),
                foundation_confidence=0.45 + (investigation_rounds * 0.14),
                clarity=VectorState(0.5 + (investigation_rounds * 0.13), f"Improved clarity after round {investigation_rounds}"),
                coherence=VectorState(0.55 + (investigation_rounds * 0.11), f"Improved coherence after round {investigation_rounds}"),
                signal=VectorState(0.45 + (investigation_rounds * 0.13), f"Improved signal after round {investigation_rounds}"),
                density=VectorState(0.6 - (investigation_rounds * 0.08), f"Improved density after round {investigation_rounds}"),  # Lower density is better
                comprehension_confidence=0.5 + (investigation_rounds * 0.12),
                state=VectorState(0.45 + (investigation_rounds * 0.15), f"Improved state awareness after round {investigation_rounds}"),
                change=VectorState(0.5 + (investigation_rounds * 0.12), f"Improved change tracking after round {investigation_rounds}"),
                completion=VectorState(0.35 + (investigation_rounds * 0.18), f"Improved completion awareness after round {investigation_rounds}"),
                impact=VectorState(0.45 + (investigation_rounds * 0.14), f"Improved impact understanding after round {investigation_rounds}"),
                execution_confidence=0.43 + (investigation_rounds * 0.16),
                uncertainty=VectorState(0.65 - (investigation_rounds * 0.15), f"Reduced uncertainty after round {investigation_rounds}"),  # Reduce uncertainty
                ...
                assessment_id=f"loop_round_{investigation_rounds}"
            )
