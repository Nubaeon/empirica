...
docker run -it -v $(pwd)/.empirica:/data/.empirica nubaeon/empirica:1.5.4-alpine /bin/sh
```
...
```bash
# CLI help
docker run --rm nubaeon/empirica:1.5.4-alpine empirica --help

# Interactive shell
docker run -it --rm nubaeon/empirica:1.5.4-alpine /bin/sh

# With persistent data volume
docker run -it \
  -v $(pwd)/.empirica:/data/.empirica \
  nubaeon/empirica:1.5.4-alpine \
  empirica session-create --ai-id docker-agent

# Check version
docker run --rm nubaeon/empirica:1.5.4-alpine empirica --version
```
...
|-----|------|------|----------|
| `1.5.4-alpine` | Alpine 3.23 | ~85MB | Production, security-hardened |
| `1.5.4` | Debian Bookworm | ~150MB | Development, glibc compatibility |

---

## Key Features in 1.5.4
