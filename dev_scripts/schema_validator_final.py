...
Identifies issues like the goals table missing status column.
"""
...
class SchemaValidator:
        ...
        if not self.db_path.exists():
            ...
            for alt_path in alt_paths:
                if alt_path.exists():
                    ...
                    break
            if not found:
                raise FileNotFoundError(f"Database not found at {self.db_path} or common locations")
    ...
    def get_expected_schema_from_code(self) -> Dict[str, List[Tuple]]:
        ...
        for py_file in py_files:
            try:
                ...
                # Find CREATE TABLE statements with regex
    ...
    def analyze_code_usage(self) -> Dict[str, Dict[str, Set[str]]]:
        ...
        for py_file in py_files:
            try:
                ...
                # Pattern for INSERT statements - capture table and potential columns
                for insert_match in re.finditer(r'INSERT\s+INTO\s+(\w+)', content, re.IGNORECASE):
                    ...
                    column_match = re.search(rf'INSERT\s+INTO\s+{table_name}\s*\(([^)]+)\)', 
                                           content[start_pos:start_pos+200], 
                                           re.IGNORECASE)
                    if column_match:
                        cols_text = column_match.group(1)
                ...
                # Pattern for SELECT statements - capture table and columns
                for select_match in re.finditer(r'SELECT\s+(.+?)\s+FROM\s+(\w+)', content, re.IGNORECASE | re.DOTALL):
                    ...
                    if '*' in col_part:
                        ...
                        for col in col_candidates:
                            ...
                            if clean_col and not any(func in clean_col.upper() for func in ['COUNT(', 'SUM(', 'AVG(', 'MAX(', 'MIN(', 'DISTINCT']):
                                # Extract just the column name part (before AS or any aliases)
                                col_name = clean_col.split(' as ')[0].split('.')[1] if '.' in clean_col.split(' as ')[0] else clean_col.split(' as ')[0]
                                col_name = col_name.split(' ')[0]  # Also handle cases like 'col_name AS alias'
                                if col_name and col_name.isidentifier():  # Basic check for valid identifier
                                    usage_stats[table_name]['select_columns'].add(col_name)
                    
                    usage_stats[table_name]['files'].add(str(py_file))
                
                # Pattern for UPDATE statements
                for update_match in re.finditer(r'UPDATE\s+(\w+)', content, re.IGNORECASE):
                    ...
                    # Look for SET clause to find update columns
                    start_pos = update_match.start()
                    update_stmt_match = re.search(rf'UPDATE\s+{table_name}\s+SET\s+([^WHERE]+)', 
                                           content[start_pos:start_pos+300], 
                                           re.IGNORECASE)
                    if update_stmt_match:
                        set_clause = update_stmt_match.group(1)
                        # Extract column names from SET clause (col = value format)
                        assignments = re.findall(r'([`\'"]?[\w_]+[`\'"]?)\s*=', set_clause)
                        for col_name_assignment in assignments:
                            col_name_clean = col_name_assignment.strip('`"\'')
    ...
    def compare_schema_and_usage(self, actual_schema: Dict[str, List[Tuple]], expected_schema: Dict[str, List[Tuple]], usage_stats: Dict[str, Dict]) -> tuple:
    ...
    def generate_report(self, actual_schema: Dict[str, List[Tuple]], expected_schema: Dict[str, List[Tuple]], usage_stats: Dict[str, Dict], issues: List[Dict], comparisons: List[Dict]) -> str:
        ...
        if issues:
            ...
            for issue in issues:
                ...
                if issue['table'] in usage_stats:
                    ...
                    report_lines.append(f"      Referenced in: {len(files_using)} files")
                report_lines.append("")
        else:
        ...
        for table_name in sorted(actual_schema.keys()):
            ...
            usage_count = 0
            if table_name in usage_stats:
                usage_count = len(usage_stats[table_name]['files'])
        ...
        if problematic_comparisons:
            ...
            for comp in problematic_comparisons:
                ...
                if comp['unused_in_db']:
                    report_lines.append(f"     • Unused in code (but in schema): {', '.join(comp['unused_in_db'])}")
    ...
    def run_validation(self) -> Dict:
        ...
        print("🔍 Comparing schemas and usage patterns...")
